-- 022: batch upsert with hash-based skip for unchanged rows.
--
-- Adds source_hash column to chargers so the batch function can detect when
-- nothing has changed and skip the expensive connectors delete/insert and
-- sources upsert, only touching last_seen_at. This cuts DB write amplification
-- significantly for warm re-ingests where most rows are unchanged.
...
--
-- Logic per element:
--   id IS NULL           → full insert (same columns as upsert_charger in 020)
--   id present AND existing source_hash = hash
--                        → touch last_seen_at only (skip connector/source writes)
--   id present AND hash differs (or no existing hash)
--                        → full update + connector replace + sources upsert
