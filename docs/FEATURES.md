...
**Address search (geocoding):** `GET /api/geocode?q=` proxies Nominatim for the "From"/"To" typeahead. Quota is **600 req/h per user** (`checkRateLimit(userId, "geocode", 600)`) — sized for a debounced typeahead where each typing pause is one request; the earlier 60/h limit was exhausted within a planning session and made every subsequent search return empty ("Locația nu a fost găsită"). The proxy forwards the user's `Accept-Language` header (so `București`, `Köln`, etc. resolve in-locale rather than forced English), requests `addressdetails=1`, and returns up to 8 matches for varied input (cities, streets, POIs, postal codes). Auth-gated; returns `{ results: [] }` on any upstream error.
