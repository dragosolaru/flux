...
- **Route accordion (mobile plan results)** (`RouteAccordion`, `expandedVariant` state): the bottom sheet is **gone in plan mode**. Each route variant is an expandable row in the top card — header shows name + time/km/stops + cost; tap to select it (highlights its polyline on the map) and expand its details inline (`CostSummary`, stops via `StopCard`, Share-to-Tesla). Only one expands at a time; the list scrolls inside `max-h-[46vh]`. Single-variant plans render details directly (no chevron). This is the "expander per variant" pattern requested over a split top+bottom layout.
