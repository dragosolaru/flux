...
**Activity panel.** Pulls 200 log rows rather than 50, and collapses repeats: rows are grouped by `level|scope|message` with a `×N` count and first-seen time. One broken source retrying all night is one line, not forty — which is what makes the larger pull readable. Ingest runs are capped at 12, enough to cover the last cron.
