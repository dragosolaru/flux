...
- **Tesla charging history: `pageNo` starts at 1.** `dx/charging/history` numbers pages from one and answers a zero with **400 "Invalid page no"** — which every sync did from August onward. It was read for weeks as the endpoint being closed to personal accounts; it was an off-by-one in one line. A genuine 403 (some charging endpoints are restricted to business fleets) is still handled separately and still reports `CHARGING_HISTORY_UNAVAILABLE`.
...
- **`/debug` is in Romanian**, including the roadmap it renders (`src/lib/roadmap.ts`). It is admin-only and read on a phone by one person, in Romanian, while it was the one page opened when something is wrong — being English made it the slowest to read. Code comments stay English.
- **`/debug` is five sections, each answering one question.** 🚗 Car (are we contacting it, is it linked, what went wrong) → ⚡ Chargers (how many, from where, do the feeds still answer) → 📋 Progress (what is left, which migrations applied) → ⚙️ Setup (which env vars the deployment can see, read-only) → 🔧 Tools (one-off probes; no badges, because nothing there describes a state). Every panel carries a one-line `purpose` saying what it is for — before that, knowing what a panel did required opening it and reading the contents, which is most of what made the page a pile rather than a tool. The sleep panel had been filed under *Progress*, whose blurb read "where the project stands, and the database schema"; Configuration sat under *Tools*, whose blurb said nothing there describes a state.
