...
**What:** Stores car-specific documents per vehicle: RCA & CASCO insurance, ITP technical inspection, rovinieta, European vignettes, bridge tolls, car tax, and `talon` (registration certificate, permanent / no expiry). Claude Vision OCR extracts plate, validity dates, issuer, amount, `seria_polita` and `bonus_malus`. Expiry status (days remaining, expired flag) is computed per document. Bank transfers ("Confirmare plată", "Ordin de plată") always classify as "other". Romanian insurer detection includes Grawe, Certasig, Axeria, plus insolvent-insurer flags.

**How to use:** UI `/documents` — vehicle selector, document cards (type badge, expiry status, plate, issuer, view/delete). Upload via the same `POST /api/documents` pipeline (OCR auto-classifies and writes a `vehicle_doc_meta` row). API: `GET /api/vehicles/[vehicleId]/vault` (returns `VaultDocument[]`, ownership-checked, rate-limited 300/hr), `POST /api/vehicles/[vehicleId]/vault` (manual entry), `PATCH /api/vehicles/[vehicleId]/vault/[documentId]` (edit metadata).
...
**Key files:** `src/types/costs.ts` (`DocumentType`, `VaultDocument`), `src/lib/ai/prompts/car-document-extraction.ts`, `src/lib/ai/document-parser.ts` (`parseCarDocument`), `src/lib/costs/processor.ts` (CAR_DOC_TYPES routing), `src/app/api/vehicles/[vehicleId]/vault/{route,calendar/route,[documentId]/route}.ts`, `src/hooks/useVaultDocuments.ts`, `src/app/(dashboard)/documents/{page.tsx,documents-client.tsx}`, `supabase/migrations/025_vehicle_doc_meta.sql`, `030_documents_manual_source.sql`.
