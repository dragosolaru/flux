...
## 27. Map Mobile — Minimal Top Card + Route Accordion

**What:** A minimal **top card** is the single control surface; plan results are an **expander per route** inside it (no bottom sheet in plan mode). The bottom sheet now only carries the explore station list.

- **Top card** (`absolute inset-x-3 top-[safe-area+12px] z-[1000] lg:hidden`): minimal rounded card. Compact mode tabs (`p-1.5`). Explore: a **single** scrollable filter row (`h-7 text-2xs` chips, power · divider · connector). Plan (editing): origin/destination `GeocodingSearch`, collapsible Advanced (SOC + vehicle), Plan button.
- **Plan-mode form collapse** (`editingRoute`): after planning the form collapses to a slim **"Origin → Destination" bar** (pencil re-opens it), so the map + route polylines stay visible.
- **Route accordion (mobile plan results)** (`RouteAccordion`, `expandedVariant` state): the bottom sheet is **gone in plan mode**. Each route variant is a compact row in the top card — name + time/km/stops + cost. **Tapping the row body selects** the route (highlights its polyline on the map); **details open only via the chevron expand selector** so casual taps never blow up the panel. Only one expands at a time; the list scrolls **vertically** inside `max-h-[38vh]` (tight rows, `py-1.5`). Expanded body: `CostSummary`, stops via `StopCard`, Share-to-Tesla. This is the "expander per variant" pattern requested over a split top+bottom layout.
- **Bottom sheet** (mobile, **explore only, opt-in**): the station list is hidden by default (markers *are* the stations). A small floating **"List · N" pill** (bottom-center) opens it; `showSheet = mode === "explore" && stationListOpen && stations.length > 0`. Snaps mid (~45 vh) / full (~88 vh), draggable; a chevron-down in the header or a downward flick (`h < midH * 0.75`) dismisses it back to the map. Slimmer mode tabs via `SegmentedControl dense` and a tighter plan form for small screens (iPhone SE / 15 Pro).
- **Desktop:** unchanged — left `DesktopSidebar` with `PlanContent` (full form + `PlanResults`).
- **Eliminated:** two stacked cards squeezing the map, duplicated route chips, dark void, "get address" overlap, map-dimming gradient.
