...
2. Update `src/lib/brands/registry.ts` to import and register the profile
3. Update `src/lib/brands/types.ts` to add the brand key to the union
4. Update `src/lib/brands/models.ts` to add the brand's models map
...
8. Update `src/components/vehicle/VehicleListCard.tsx` color + label maps
9. (Live only) Implement `src/lib/<brand>/api.ts` + `src/app/api/<brand>/` OAuth routes
10. Add to `LIVE_INTEGRATIONS` env var to enable live mode
